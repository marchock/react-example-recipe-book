import { combineReducers } from 'redux';
import { reducer as formReducer } from 'redux-form';

const recipes = [
    {
        name: 'Double bean & roasted pepper chilli',
        description: 'This warming vegetarian chilli is a low-fat, healthy option that packs in the veggies and flavour. Serve with Tabasco sauce, soured cream or yogurt',
        img: 'https://www.bbcgoodfood.com/sites/default/files/styles/recipe/public/recipe/recipe-image/2016/11/chilli.jpg?itok=WBfgJnpV',
        ingredients: [
            {
                ingredient: 'Onions',
                amount: 2
            },
            {
                ingredient: 'celery sticks',
                amount: 2
            },
            {
                ingredient: 'yellow or orange peppers',
                amount: 2
            },
            {
                ingredient: 'tsp chipotle paste',
                amount: '2'
            },
            {
                ingredient: 'red wine vinegar',
                amount: '2 tbsp'
            },
            {
                ingredient: 'cocoa powder',
                amount: '1 tbsp'
            },
            {
                ingredient: 'dried oregano',
                amount: '1 tbsp'
            },
            {
                ingredient: 'sweet smoked paprika',
                amount: '1 tbsp'
            },
            {
                ingredient: 'ground cumin',
                amount: '2 tbsp'
            },
            {
                ingredient: 'ground cinnamon',
                amount: '1 tbsp'
            },
            {
                ingredient: '400g cans chopped tomatoes',
                amount: '2'
            },
            {
                ingredient: '400g can refried beans',
                amount: '1'
            }
        ]
    },
    {
        name: 'Spiced chicken, spinach & sweet potato stew',
        description: 'A healthy one-pot with warming spices thats low fat, low calorie, 3 of your 5-a-day and also full of fibre!',
        img: 'https://www.bbcgoodfood.com/sites/default/files/styles/recipe/public/recipe/recipe-image/2016/04/spiced-chicken-spinach-sweet-potato-stew.jpg?itok=jsrhL11K',
        ingredients: [
            {
                ingredient: 'yellow or orange peppers',
                amount: 2
            },
            {
                ingredient: 'tsp chipotle paste',
                amount: '2'
            },
            {
                ingredient: 'red wine vinegar',
                amount: '2 tbsp'
            },
            {
                ingredient: 'cocoa powder',
                amount: '1 tbsp'
            },
            {
                ingredient: 'dried oregano',
                amount: '1 tbsp'
            },
            {
                ingredient: 'yellow or orange peppers',
                amount: 2
            },
            {
                ingredient: 'tsp chipotle paste',
                amount: '2'
            },
            {
                ingredient: 'red wine vinegar',
                amount: '2 tbsp'
            },
            {
                ingredient: 'cocoa powder',
                amount: '1 tbsp'
            },
            {
                ingredient: 'dried oregano',
                amount: '1 tbsp'
            }
        ]
    },
    {
        name: 'Zesty salmon with roasted beets & spinach',
        description: 'This fresh and filling salmon supper is packed full of heart healthy ingredients, is rich in calcium, iron and vitamin C and makes up 4 of your 5 a day',
        img: 'https://www.bbcgoodfood.com/sites/default/files/styles/recipe/public/recipe_images/salmon-main.jpg?itok=MDGKkzor',
        ingredients: [
            {
                ingredient: 'yellow or orange peppers',
                amount: 2
            },
            {
                ingredient: 'tsp chipotle paste',
                amount: '2'
            },
            {
                ingredient: 'red wine vinegar',
                amount: '2 tbsp'
            },
            {
                ingredient: 'cocoa powder',
                amount: '1 tbsp'
            },
            {
                ingredient: 'dried oregano',
                amount: '1 tbsp'
            },
            {
                ingredient: 'yellow or orange peppers',
                amount: 2
            },
            {
                ingredient: 'tsp chipotle paste',
                amount: '2'
            },
            {
                ingredient: 'red wine vinegar',
                amount: '2 tbsp'
            },
            {
                ingredient: 'cocoa powder',
                amount: '1 tbsp'
            },
            {
                ingredient: 'dried oregano',
                amount: '1 tbsp'
            }
        ]
    },
    {
        name: 'Fish stew with roast garlic & saffron',
        description: 'Try this simpler, lighter version of bouillabaisse, a classic fish soup - serve with toasted slices of baguette to mop up the tomato based sauce',
        img: 'https://www.bbcgoodfood.com/sites/default/files/styles/recipe/public/recipe_images/fish-stew-roast-garlic-saffron.jpg?itok=KJ_W0Ez-',
        ingredients: [
            {
                ingredient: 'yellow or orange peppers',
                amount: 2
            },
            {
                ingredient: 'tsp chipotle paste',
                amount: '2'
            },
            {
                ingredient: 'red wine vinegar',
                amount: '2 tbsp'
            },
            {
                ingredient: 'cocoa powder',
                amount: '1 tbsp'
            },
            {
                ingredient: 'dried oregano',
                amount: '1 tbsp'
            },
            {
                ingredient: 'yellow or orange peppers',
                amount: 2
            },
            {
                ingredient: 'tsp chipotle paste',
                amount: '2'
            },
            {
                ingredient: 'red wine vinegar',
                amount: '2 tbsp'
            },
            {
                ingredient: 'cocoa powder',
                amount: '1 tbsp'
            },
            {
                ingredient: 'dried oregano',
                amount: '1 tbsp'
            }
        ]
    },
    {
        name: 'Spaghetti with smoky tomato & seafood sauce',
        description: 'Chilli, fennel seeds and smoked paprika add warmth and flavour to a rich tomato sauce, served with seafood in this quick pasta dish',
        img: 'https://www.bbcgoodfood.com/sites/default/files/styles/recipe/public/user-collections/my-colelction-image/2015/12/spaghetti-smoky-tomato-seafood-sauce.jpg?itok=C7x-UL_I',
        ingredients: [
            {
                ingredient: 'yellow or orange peppers',
                amount: 2
            },
            {
                ingredient: 'tsp chipotle paste',
                amount: '2'
            },
            {
                ingredient: 'red wine vinegar',
                amount: '2 tbsp'
            },
            {
                ingredient: 'cocoa powder',
                amount: '1 tbsp'
            },
            {
                ingredient: 'dried oregano',
                amount: '1 tbsp'
            },
            {
                ingredient: 'yellow or orange peppers',
                amount: 2
            },
            {
                ingredient: 'tsp chipotle paste',
                amount: '2'
            },
            {
                ingredient: 'red wine vinegar',
                amount: '2 tbsp'
            },
            {
                ingredient: 'cocoa powder',
                amount: '1 tbsp'
            },
            {
                ingredient: 'dried oregano',
                amount: '1 tbsp'
            }
        ]
    }
];







const rootReducer = combineReducers({
    recipe: {
        list: [],
        selected: null,
    },
    form: formReducer
});

export default rootReducer;
